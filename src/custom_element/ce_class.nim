...
macro class*(arg1, arg2: untyped): untyped =
  ...
  """.unindent() % [
    observableProperties.map(
      proc(observableProperty: tuple[name: NimNode, kind: NimNode,
          initialValue: NimNode]): string =
    result = "@observable " & $(observableProperty.name) & " = " & $(toStrLit(
        observableProperty.initialValue)) & ";"
  ).join("\n"),
    jsConstructorParams.join(", "),
    if len(parentClassName) > 0: "super($1);" % jsConstructorParams.join(
        ", ") else: "",
